...
import { selectCourse, selectSectionList, setCourse, setSectionList } from "../../../redux/reducers/courseManager";
...
import { DashboardInfo } from "../../../routes/adminDashboard";
import { CourseEditDashboardContainer, ListboxButtonStyled, ListboxOptionsStyled, ListboxOptionStyled, SectionButton } from "./course-edit-dashboard.styles";
...
function CourseEditDashboard(){
    ...
    const [stackCategories, setStackCategories] = useState<StackCategories>('FRONT_END')
    ...
    function addSection(){
        dispatch(setSectionList([...lessonList, {lessons: [] as Lesson[], name: "new Section"} as Section]))
    }

    return(
        <CourseEditDashboardContainer>
            <input value={course.title} onChange={(event) => name(event.target.value)} />
            <textarea value={course.description} onChange={(event) => description(event.target.value)} />
            ...
            <DashboardInfo>
                {
                    sections.map((section: Section, index) => (<SectionPannel section={section} sectionLocation={index+1} key={index}/>))
                }
