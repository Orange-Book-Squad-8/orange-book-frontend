...
import { DisclosureButtonEditStyled, DisclosureButtonStyled, DisclosureStyled, FormDisclosureButtonEditStyled, MinDIv } from "./section-pannel.styles";
...
function SectionPannel({section, sectionLocation}: SectionPannelProps) {
  ...
  function editName(name: string){
    const sections: Section[] = produce(
      sectionList, (draft) => {
        draft.splice(
          sectionLocation,
          1,
    ...
    <Disclosure>
        <DisclosureStyled>
          <FormDisclosureButtonEditStyled onSubmit={(event) => finishEdit(event)} >
            {
              iseditingName ? 
              <DisclosureButtonEditStyled value={section.name} onChange={(event) => editName(event.target.value) }/> :
              (
        ...
        <Disclosure.Panel>
          {
            section.lessons.map(
              (lesson: Lesson, index: number) => (<ItemCard lesson={lesson} listIndex={index} lessonLocation={sectionLocation} key={index}/>)
            )
          }
          <MinDIv ref={dropRef}>
            <p>Arraste aqui</p>
